...
STUDENT_WEIGHTS = "best_student_nclt_4.pt"
...
ALPHA_END    = 0.20        # gain distillation weight, epoch EPOCHS
...
def _pos_vel_smoothl1(pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
    """
    Combined loss:
      - SmoothL1 per component (stable gradients)
      - RMSE of Euclidean position error (directly optimises the eval metric)
    Position weighted POS_WEIGHT× velocity.
    """
    loss_pos  = nn.SmoothL1Loss()(pred[:, :, :3], target[:, :, :3])
    loss_vel  = nn.SmoothL1Loss()(pred[:, :, 3:],  target[:, :, 3:])
    # Euclidean position RMSE — directly matches evaluation metric
    pos_dist  = torch.norm(pred[:, :, :3] - target[:, :, :3], dim=2)  # (B, T)
    loss_rmse = torch.sqrt(torch.mean(pos_dist ** 2) + 1e-8)
    return POS_WEIGHT * loss_pos + loss_vel + 0.5 * loss_rmse
...
def train():
    ...
    teacher.load_state_dict(torch.load(TEACHER_WEIGHTS, map_location=DEVICE, weights_only=False))
    teacher.eval()
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        beta  = BETA_END + 0.5 * (BETA_START - BETA_END) * (1 + np.cos(np.pi * t))
        ...
        # Early stop if RMSE explodes past 3× best (collapse detection)
        elif val_rmse > 3.0 * best_val_rmse and epoch > 30:
            print(f"  [Early stop] RMSE {val_rmse:.4f} > 3× best {best_val_rmse:.4f}")
            break_val_rmse = val_rmse
            torch.save(student.state_dict(), STUDENT_WEIGHTS)
            saved = f"  ← saved (RMSE={val_rmse:.4f} m)"
    ...
    student.load_state_dict(torch.load(STUDENT_WEIGHTS, map_location=DEVICE, weights_only=False))
    student.eval()
    ...
    pos_err   = all_preds[:, :, :3] - all_trues[:, :, :3]   # (N, T, 3)
    pos_dist  = torch.norm(pos_err, dim=2)                    # (N, T) Euclidean per step
    rmse_pos  = float(torch.sqrt(torch.mean(pos_dist ** 2)).item())   # matches evaluate_models
    mae_pos   = float(torch.mean(pos_dist).item())                    # Euclidean MAE
    prec_1m   = float((pos_dist < 1.0).float().mean().item() * 100)
